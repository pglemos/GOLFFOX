...
class DriverService extends StateNotifier<AsyncValue<List<Driver>>> {
    ...
    try {
      ...
      if (_currentFilters.search?.isNotEmpty ?? false) {
        ...
        filteredDrivers = filteredDrivers
            .where((driver) =>
                driver.name.toLowerCase().contains(search) ||
                driver.email.toLowerCase().contains(search) ||
                (driver.phone?.toLowerCase().contains(search) ?? false) ||
                (driver.cpf?.toLowerCase().contains(search) ?? false))
            .toList();
      }
  ...
  List<Driver> getOnlineDrivers() =>
      _drivers.where((driver) => driver.isOnline).toList();

  // Buscar motoristas com alertas
  List<Driver> getDriversWithAlerts() =>
      _drivers.where((driver) => driver.hasAlerts).toList();
