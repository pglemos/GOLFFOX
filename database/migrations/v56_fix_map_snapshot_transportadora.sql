-- ========================================
-- Migration v56: Fix gf_map_snapshot_full para suportar transportadora_id
-- ========================================
-- 
-- Esta migration atualiza a função gf_map_snapshot_full para:
-- 1. Aceitar parâmetro p_transportadora_id
-- 2. Filtrar veículos e rotas por transportadora_id quando fornecido
-- 3. Corrigir qualquer referência incorreta a colunas da view v_driver_last_position
--

-- Atualizar função gf_map_snapshot_full para aceitar p_transportadora_id
...
BEGIN
  ...
  WHERE t.status IN ('scheduled', 'inProgress')
    ...
    AND (p_transportadora_id IS NULL OR v.transportadora_id = p_transportadora_id OR r.transportadora_id = p_transportadora_id)
  ...
  -- GARAGES: Veículos parados sem rota ativa (filtrar por transportadora_id se fornecido)
  SELECT json_agg(
    json_build_object(
      'id', v.id,
      'vehicle_id', v.id,
  ...
  WHERE v.id NOT IN (
    ...
    AND (p_transportadora_id IS NULL OR v.transportadora_id = p_transportadora_id);

  -- ROUTES: Info de rota para desenhar polyline (filtrar por transportadora_id se fornecido)
  SELECT json_agg(
    json_build_object(
      'id', r.id,
      'route_id', r.id,
  ...
  WHERE (p_company_id IS NULL OR r.company_id = p_company_id)
    AND (p_transportadora_id IS NULL OR r.transportadora_id = p_transportadora_id)
...
COMMENT ON FUNCTION public.gf_map_snapshot_full(UUID, UUID, UUID) IS 
  'Retorna snapshot completo do mapa. Aceita p_company_id, p_transportadora_id e p_route_id. Passe NULL para parâmetros não utilizados.';

-- Grant execute permission
GRANT EXECUTE ON FUNCTION public.gf_map_snapshot_full(UUID, UUID, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.gf_map_snapshot_full(UUID, UUID, UUID) TO anon;
