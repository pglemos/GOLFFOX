...
-- Baseado em v43_admin_views.sql, mas ajustado para estrutura correta das tabelas
...
SELECT 
  ...
  t.started_at::time AS actual_time,
  CASE 
...
SELECT 
  ...
  CASE 
    ...
    THEN ROUND((COUNT(DISTINCT tp.passenger_id)::NUMERIC / MAX(v.capacity)) * 100, 2)
    ELSE 0
...
SELECT 
  ...
  CASE 
    WHEN tp.status = 'cancelled' THEN 'Cancelado pelo passageiro'
    WHEN tp.status = 'no_show' THEN 'Não compareceu'
    WHEN tp.status = 'rejected' THEN 'Rejeitado'
    WHEN tp.status = 'pending' THEN 'Pendente'
    WHEN t.completed_at IS NULL THEN 'Viagem não completada'
    ELSE 'Não embarcado'
  END AS reason
FROM public.trips t
JOIN public.routes r ON t.route_id = r.id
...
WHERE t.scheduled_at >= CURRENT_DATE - INTERVAL '90 days'
  AND r.company_id IS NOT NULL
  AND (
    tp.status IN ('cancelled', 'no_show', 'rejected', 'pending')
    OR tp.status IS NULL
    OR (t.status = 'completed' AND tp.status NOT IN ('picked_up', 'pickedup', 'completed', 'dropped_off'))
  );
...
SELECT 
  ...
  ) FILTER (WHERE t.status = 'completed' AND t.completed_at IS NOT NULL) AS avg_delay
FROM public.trips t
JOIN public.routes r ON t.route_id = r.id
WHERE t.scheduled_at >= CURRENT_DATE - INTERVAL '90 days'
  AND r.company_id IS NOT NULL
