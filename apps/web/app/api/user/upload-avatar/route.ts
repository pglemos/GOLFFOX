...
async function uploadAvatarHandler(req: NextRequest) {
  ...
  try {
    ...
    if (uploadError) {
      ...
      if (uploadError.message.includes('Bucket not found') || uploadError.message.includes('not found') || (uploadError as { statusCode?: string | number }).statusCode === '404') {
...
export const POST = withRateLimit(uploadAvatarHandler, 'api')
