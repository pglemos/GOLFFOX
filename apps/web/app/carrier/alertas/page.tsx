...
export default function CarrierAlertasPage() {
  ...
  const loadAlertas = async () => {
    try {
      // Buscar alertas relacionados à transportadora
      ...
      if (userData?.carrier_id) {
        query = query.or(`carrier_id.eq.${userData.carrier_id},carrier_id.is.null`)
      }
    ...
    <AppShell user={{ id: user?.id || "", name: user?.name || "Transportadora", email: user?.email || "", role: "carrier" }}>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-[var(--ink-muted)]">Notificações e alertas da transportadora</p>
        </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <Badge variant={alerta.severity === 'critical' ? 'destructive' : 'default'}>
