        ...
        <AppShell user={{ id: user.id, name: user.name || "Admin", email: user.email, role: user.role || "admin", avatar_url: user.avatar_url }}>
            <div className="space-y-6">
                <div className="flex items-center gap-3">
                    <BarChart3 className="h-8 w-8 text-[var(--brand)]" />
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-bold">Analytics</h1>
                        <p className="text-sm text-[var(--ink-muted)]">Métricas e insights do sistema</p>
                    </div>
                </div>

                <Card className="p-8 sm:p-12 text-center">
                    <Construction className="h-16 w-16 mx-auto mb-4 text-[var(--brand)] opacity-50" />
