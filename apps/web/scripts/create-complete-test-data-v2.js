 ...
 * Versão 2: Com suporte a carrier_id
 */
...
async function createTestData() {
  ...
  console.log('🎨 CRIANDO DADOS DE TESTE COMPLETOS (V2)\n');
  console.log('═══════════════════════════════════════\n');
  ...
  try {
    ...
    // 1. Pegar empresa e carrier
    console.log('1️⃣  Obtendo empresa e carrier...');
    ...
    // Pegar ou criar carrier
    let carrierId;
    const carriers = await client.query(`SELECT id, name FROM carriers LIMIT 1`);
    if (carriers.rows.length > 0) {
      carrierId = carriers.rows[0].id;
      console.log(`✅ Carrier: ${carriers.rows[0].name} (${carrierId})\n`);
    } else {
      // Criar carrier de teste
      const carrierTestId = '00000000-0000-0000-0000-000000000002';
      await client.query(`
        INSERT INTO carriers (id, name, created_at)
        VALUES ($1, 'Transportadora Teste', NOW())
        ON CONFLICT (id) DO NOTHING
      `, [carrierTestId]);
      carrierId = carrierTestId;
      console.log(`✅ Carrier criado: Transportadora Teste (${carrierId})\n`);
    }
    ...
    for (const vehicle of vehiclesToCreate) {
      await client.query(`
        INSERT INTO vehicles (id, plate, model, year, capacity, is_active, company_id, carrier_id, created_at)
      ...
      `, [vehicle.id, vehicle.plate, vehicle.model, vehicle.year, vehicle.capacity, companyId, carrierId]);
      
      console.log(`   ✓ ${vehicle.plate} (${vehicle.model})`);
    }
    console.log('');

    // 3. Criar rota de teste com carrier_id
