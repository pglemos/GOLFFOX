...
-- - Vehicles (gf_veiculo_documents)
-- - Drivers compensation (gf_motorista_compensation)
-- - Carriers (gf_transportadora_documents)
-- Plus expands existing tables with banking and legal rep info

-- ====================================================
-- PART 1: veiculo DOCUMENTS TABLE
-- ====================================================
CREATE TABLE IF NOT EXISTS public.gf_veiculo_documents (
...
CREATE INDEX IF NOT EXISTS idx_gf_vehicle_docs_vehicle_id ON public.gf_veiculo_documents(veiculo_id);
CREATE INDEX IF NOT EXISTS idx_gf_vehicle_docs_type ON public.gf_veiculo_documents(document_type);
CREATE INDEX IF NOT EXISTS idx_gf_vehicle_docs_expiry ON public.gf_veiculo_documents(expiry_date);
CREATE INDEX IF NOT EXISTS idx_gf_vehicle_docs_status ON public.gf_veiculo_documents(status);

COMMENT ON TABLE public.gf_veiculo_documents IS 'Documentos de veículos (CRLV, ANTT, seguros, etc.)';
...
-- ====================================================
CREATE TABLE IF NOT EXISTS public.gf_motorista_compensation (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_gf_driver_compensation_driver ON public.gf_motorista_compensation(motorista_id);
CREATE INDEX IF NOT EXISTS idx_gf_driver_compensation_active ON public.gf_motorista_compensation(motorista_id) WHERE is_active = true;

COMMENT ON TABLE public.gf_motorista_compensation IS 'Salários e benefícios dos motoristas';

-- ====================================================
-- PART 4: transportadora DOCUMENTS TABLE
-- ====================================================
CREATE TABLE IF NOT EXISTS public.gf_transportadora_documents (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  transportadora_id UUID NOT NULL REFERENCES public.carriers(id) ON DELETE CASCADE,
  document_type VARCHAR(50) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_gf_carrier_docs_carrier_id ON public.gf_transportadora_documents(transportadora_id);
CREATE INDEX IF NOT EXISTS idx_gf_carrier_docs_type ON public.gf_transportadora_documents(document_type);
CREATE INDEX IF NOT EXISTS idx_gf_carrier_docs_expiry ON public.gf_transportadora_documents(expiry_date);
CREATE INDEX IF NOT EXISTS idx_gf_carrier_docs_status ON public.gf_transportadora_documents(status);

COMMENT ON TABLE public.gf_transportadora_documents IS 'Documentos de transportadoras (contratos, certidões, etc.)';
...
-- PART 6: RLS POLICIES
-- ====================================================
ALTER TABLE public.gf_veiculo_documents ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.gf_motorista_compensation ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.gf_transportadora_documents ENABLE ROW LEVEL SECURITY;

-- veiculo Documents Policies
DROP POLICY IF EXISTS "Service role full access on gf_veiculo_documents" ON public.gf_veiculo_documents;
CREATE POLICY "Service role full access on gf_veiculo_documents" 
  ON public.gf_veiculo_documents FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Users can read vehicle_documents for their company" ON public.gf_veiculo_documents;
CREATE POLICY "Users can read vehicle_documents for their company" 
  ON public.gf_veiculo_documents FOR SELECT TO authenticated 
  USING (
    veiculo_id IN (
...
DROP POLICY IF EXISTS "Service role full access on gf_motorista_compensation" ON public.gf_motorista_compensation;
CREATE POLICY "Service role full access on gf_motorista_compensation" 
  ON public.gf_motorista_compensation FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Drivers can read their own compensation" ON public.gf_motorista_compensation;
CREATE POLICY "Drivers can read their own compensation" 
  ON public.gf_motorista_compensation FOR SELECT TO authenticated USING (motorista_id = auth.uid());

DROP POLICY IF EXISTS "Admins can manage motorista compensation" ON public.gf_motorista_compensation;
CREATE POLICY "Admins can manage motorista compensation"
  ON public.gf_motorista_compensation FOR ALL TO authenticated
...
DROP POLICY IF EXISTS "Service role full access on gf_transportadora_documents" ON public.gf_transportadora_documents;
CREATE POLICY "Service role full access on gf_transportadora_documents" 
  ON public.gf_transportadora_documents FOR ALL TO service_role USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Users can read carrier_documents for their transportadora" ON public.gf_transportadora_documents;
CREATE POLICY "Users can read carrier_documents for their transportadora" 
  ON public.gf_transportadora_documents FOR SELECT TO authenticated 
  USING (transportadora_id IN (SELECT transportadora_id FROM users WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Admins can manage transportadora documents" ON public.gf_transportadora_documents;
CREATE POLICY "Admins can manage transportadora documents"
  ON public.gf_transportadora_documents FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'operador')))
  WITH CHECK (EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'operador')));
...
-- ====================================================
DROP TRIGGER IF EXISTS update_gf_vehicle_docs_updated_at ON public.gf_veiculo_documents;
CREATE TRIGGER update_gf_vehicle_docs_updated_at 
  BEFORE UPDATE ON public.gf_veiculo_documents
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_gf_driver_compensation_updated_at ON public.gf_motorista_compensation;
CREATE TRIGGER update_gf_driver_compensation_updated_at 
  BEFORE UPDATE ON public.gf_motorista_compensation
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_gf_carrier_docs_updated_at ON public.gf_transportadora_documents;
CREATE TRIGGER update_gf_carrier_docs_updated_at 
  BEFORE UPDATE ON public.gf_transportadora_documents
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
SELECT 
  ...
  v.company_id,
  v.transportadora_id
FROM public.gf_veiculo_documents vd
JOIN public.vehicles v ON vd.veiculo_id = v.id
...
JOIN public.users u ON dd.motorista_id = u.id
...
SELECT 
  ...
  cd.transportadora_id as entity_id,
  ...
  END as alert_level,
  NULL as company_id,
  cd.transportadora_id as transportadora_id
FROM public.gf_transportadora_documents cd
JOIN public.carriers c ON cd.transportadora_id = c.id
