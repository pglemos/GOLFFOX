-- Script para verificar e criar views de relatórios
-- Executa as views do arquivo v43_admin_views.sql
...
SELECT 
  ...
  CASE 
    ...
    THEN ROUND((COUNT(DISTINCT tp.id)::NUMERIC / MAX(v.capacity)) * 100, 2)
    ELSE 0
...
WHERE t.scheduled_at >= CURRENT_DATE - INTERVAL '90 days'
  AND r.company_id IS NOT NULL
  AND tp.status IN ('cancelled', 'no_show', 'rejected', 'pending')
  AND tp.status != 'picked_up';
