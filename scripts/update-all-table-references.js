/**
 * Script para atualizar TODAS as referências de tabelas do Supabase no código
 * de inglês para português (PT-BR)
 * 
 * Tabelas a renomear:
 * - carriers → transportadoras
 * - drivers → motoristas (se for tabela separada)
 * - vehicles → veiculos
...
const IGNORE_PATTERNS = [
  ...
  /coverage/,
  /\.cache/,
  /migrations\/.*\.sql$/, // Ignorar arquivos SQL de migrations (já foram atualizados)
]
...
function main() {
  console.log('🔄 Atualizando referências de tabelas do Supabase para PT-BR...\n')
  ...
  if (totalModified > 0) {
    console.log('\n✅ Referências de tabelas atualizadas com sucesso!')
    console.log('\n📝 Próximos passos:')
    console.log('   1. Revisar as mudanças')
    console.log('   2. Aplicar a migration SQL no Supabase')
    console.log('   3. Testar todas as funcionalidades')
  } else {
    console.log('\nℹ️  Nenhuma referência encontrada para atualizar.')
  }
}
